...
Le workflow contient aussi le job `sidestore-ipa`. Il compile une version iPhone seule pour appareil réel et publie l’artefact `FieldPulse-HeadMouse-SideStore`, qui contient `FieldPulseHeadMouse.ipa`. Dans GitHub : **Actions > Build FieldPulse > Run workflow**, attends le job `sidestore-ipa`, puis télécharge cet artefact et importe l’IPA dans SideStore. L’app s’affiche sous le nom `FieldPulse Head Mouse` et utilise le Bundle ID `com.example.fieldpulse.headmouse` par défaut.
